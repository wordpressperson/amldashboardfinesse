...
# --- Aggressive Custom CSS for Forced Dark Mode ---
st.markdown("""
    <style>
    ...
    /* Force default text to be light */
    html, body, [class*="css"] {
    ...
    /* Style the metric cards (Targeting multiple potential Streamlit class names) */
...
# --- Dashboard Layout ---
st.markdown("<h1 class='demo-title'>AML Transaction Risks Monitoring</h1>", unsafe_allow_html=True)
...
with col_chart1:
    st.markdown("<h4 style='color: #FFFFFF;'>Recent Activity Flow</h4>", unsafe_allow_html=True)
    fig_bar = go.Figure()
    ...
    # Update axes colors to show up on dark background
    fig_bar.update_xaxes(showgrid=False, linecolor='#333')
    fig_bar.update_yaxes(showgrid=True, gridcolor='#333', linecolor='#333')
    
    st.plotly_chart(fig_bar, use_container_width=True, config={'displayModeBar': False})

with col_chart2:
    st.markdown("<h4 style='color: #FFFFFF;'>Verification Status</h4>", unsafe_allow_html=True)
    ...
    fig_donut.update_layout(
        ...
        annotations=[dict(text='250<br>Alerts', x=0.5, y=0.5, font_size=20, showarrow=False, font_color='#FFFFFF')]
    )
    st.plotly_chart(fig_donut, use_container_width=True, config={'displayModeBar': False})

# 3. Bottom Row: Ongoing Investigations Table
st.markdown("<h4 style='color: #FFFFFF; margin-top: 20px;'>High-Priority Alerts & Investigations</h4>", unsafe_allow_html=True)
